...
def intersectionPercentage(a, b):  # returns None if rectangles don't intersect
    ...
    if (xO >= 0) and (yO >= 0):
        percent = interArea / targetArea * 100
        percents.append(percent)
        print "Percent Coverage: {0:.0f}%".format(percent)
...
def plotRectangles(coorSpace, tBB, creatures):
    ...
    for cre in creatures:
        ...
        rect = patches.Rectangle((tBB["X"], tBB["Y"]), tBB["W"], tBB["H"], linewidth=tBB["l"], edgecolor='b',
                                 facecolor='none')
        ax.add_patch(rect)
...
# makes chart of coverage area by generation
def line_plot(percents):
    x = range(len(percents))
    plt.xlabel("X-axis")
    plt.ylabel("Y-axis")
    plt.title("Coverage Area Percentage")
    for i in range(len(percents)):
        print x[i], percents[i]
        plt.plot(x, percents)
    plt.legend()
    plt.show()
